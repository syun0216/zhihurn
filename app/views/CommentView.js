...
export default class CommentView extends Component {
    ...
    _renderLongCommentView() {
        return this.state.longComments.map((item,idx) => {
            return this._renderCommonCommentView(item,idx);
        })
    }

    _renderShortCommentView() {
        return this.state.shortComments.map((item, idx) => {
            return this._renderCommonCommentView(item, idx);
        })
    }
    ...
    _renderCommonCommentView(data, key) {
        return (
            <View key={`${key}`} style={{flex: 1, padding:10,flexDirection:'row',borderBottomWidth: 1, borderColor: '#ccc',backgroundColor:'white'}}>
                <View style={{width: 50}}>
                    <Image style={{width: 34, height: 34, borderRadius: 20}} source={{uri: data.avatar}}/>
                </View>
                <View style={{flex: 1,paddingRight:5,marginTop:5}}>
                    <View style={{height: 10,flex:1,flexDirection:'row',alignItems:'center',justifyContent:'space-between',marginBottom:10}}>
